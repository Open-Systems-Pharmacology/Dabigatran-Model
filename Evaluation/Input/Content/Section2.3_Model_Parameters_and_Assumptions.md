...
For the model parameters `Specific intestinal permeability` and `Specific organ permeability` calculated values were used for all three compounds. Instead, the `Lipophilicity` values were optimized to best match clinical data (see Section 2.3.4). Parameter optimizations of logP values (iv first, then oral) were informed by clinical dabigatran etexilate, dabigatran and total dabigatran (sum of dabigatran + dabigatran glucuronide) plasma concentration data. Parameter optimization of the Pgp kcat value was informed by addition of the Rifampicin-Dabigatran Pgp-induction DDI trial data. 
...
`Lipophilicity` values were optimized to best match clinical data (see "Absorption"); for `Specific organ permeability` the calculated values were used for all three compounds. 
